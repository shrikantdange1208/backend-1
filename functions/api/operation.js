...
router.post('/', isAdminOrSuperAdmin, async (request, response, next) => {
    ...
    // Add event in Audit
    const eventMessage = `User ${req.user.name} created new operation ${operationName}`
...
router.put('/', isAdminOrSuperAdmin, async (request, response, next) => {
    ...
    // Add event in Audit
    const eventMessage = `User ${req.user.name} updated operation ${oldData[constants.NAME]}`
...
router.delete('/:id', isSuperAdmin, async(request, response, next) => {
    ...
    // Add event in Audit
    const eventMessage = `User ${req.user.name} deleted operation ${operationData[constants.NAME]}`
    audit.logEvent(eventMessage, request)
