...
router.post('/', isAdminOrSuperAdmin, async (request, response, next) => {
    ...
    // Add event in Audit
    const eventMessage = `User ${req.user.name} added new category ${categoryName}`
...
router.put('/', isAdminOrSuperAdmin, async (request, response, next) => {
    ...
    // Add event in Audit
    const eventMessage = `User ${req.user.name} updated category ${oldData[constants.NAME]}`
...
router.delete('/:id', isSuperAdmin, async (request, response, next) => {
    ...
    // Add event in Audit
    const eventMessage = `User ${req.user.name} deleted category ${categoryData[constants.NAME]}`
    audit.logEvent(eventMessage, request)
